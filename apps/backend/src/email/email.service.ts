...
export class EmailService {
  ...
  // Invoice email — SAME mechanism as sendReceipt (Resend + PDF attachment +
  // { id } return so the caller flips SENT/SEND_FAILED), only the copy differs.
  // Reuses ReceiptPayload (receiptNumber carries the INV- number).
  async sendInvoice(payload: ReceiptPayload): Promise<{ id: string }> {
    if (!this.resend) {
      this.logger.error(
        `[EmailService] Cannot send invoice ${payload.receiptNumber} to ${payload.to} — Resend not configured`,
      );
      throw new Error(
        'Email sending is not configured (RESEND_API_KEY missing)',
      );
    ...
    const { data, error } = await this.resend.emails.send({
      from: this.from,
      to: payload.to,
      subject: `Invoice ${payload.receiptNumber} from ${payload.companyName}`,
      html: this.buildInvoiceHtml(payload),
      attachments: [
        {
          filename: `${payload.receiptNumber}.pdf`,
          content: payload.pdfBuffer,
    ...
    if (error) {
      this.logger.error(
        `[EmailService] Failed to send invoice ${payload.receiptNumber} to ${payload.to}: ${JSON.stringify(error)}`,
      );
      throw new Error(`Email delivery failed: ${error.message}`);
    }

    this.logger.log(
      `[EmailService] Invoice ${payload.receiptNumber} sent to ${payload.to} (id: ${data?.id})`,
    );
    return { id: data?.id ?? '' };
  }
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  ...
  // Invoice email HTML — mirrors buildReceiptHtml's branding, invoice copy.
  private buildInvoiceHtml(p: ReceiptPayload): string {
    const appUrl = this.getAppUrl();
    const logoUrl = `${appUrl}/ntssign-logo-light.svg`;
    const company = escapeHtml(p.companyName);
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(2,41,119,0.08);">
            ...
            <td style="background:linear-gradient(135deg,#05a5ff 0%,#022977 60%,#0400f0 100%);padding:32px 40px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td align="right">
                    <div style="background:rgba(255,255,255,0.18);border:1px solid rgba(255,255,255,0.35);border-radius:8px;padding:6px 12px;color:rgba(255,255,255,0.9);font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;">Invoice</div>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              ...
              <p style="margin:0 0 28px;color:#4b5563;font-size:15px;line-height:1.6;">
                Your invoice from ${company} is attached to this email as a PDF.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;margin-bottom:28px;">
                <tr>
                  <td style="padding:20px 24px;">
                    <div style="color:#6b7280;font-size:11px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;">Invoice</div>
                    <div style="color:#111827;font-size:16px;font-weight:600;">${escapeHtml(p.receiptNumber)}</div>
                    <div style="color:#6b7280;font-size:12px;margin-top:4px;">${company}</div>
                  </td>
                </tr>
