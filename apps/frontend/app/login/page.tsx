  ...
  return (
    <main className="relative h-screen overflow-hidden bg-[color:var(--background)] px-3 py-3 sm:px-4 sm:py-4 lg:px-6 lg:py-6">
      ...
      <div className="mx-auto flex h-[calc(100vh-1.5rem)] w-full max-w-7xl items-center justify-center bg-transparent p-0 sm:h-[calc(100vh-2rem)] sm:p-3 md:h-screen md:py-[19vh] lg:h-[calc(100vh-3rem)] lg:p-5">
        <div className="relative grid h-full w-full overflow-hidden bg-transparent shadow-none sm:rounded-[2rem] sm:border sm:border-[color:var(--brand-secondary)] sm:dark:border-[color:var(--border-strong)] sm:bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] sm:dark:bg-[image:var(--bg-shell-inner)] sm:shadow-[0_26px_80px_rgba(2,41,119,0.18)] sm:dark:shadow-[var(--shadow-strong)] md:grid-cols-[1fr_1fr] xl:grid-cols-[1.05fr_0.95fr]">
          <div className="absolute right-4 top-4 z-20 md:right-5 md:top-5">
            <ThemeToggle />
          </div>

          <section className="relative hidden min-h-[24rem] border-r border-[color:var(--brand-secondary)] dark:border-[color:var(--border-strong)] bg-white dark:bg-[color:var(--bg-surface)] md:block md:h-full md:min-h-0">
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(247,251,255,0.98))] dark:bg-[radial-gradient(circle_at_top_left,rgba(5,165,255,0.08),transparent_26%),radial-gradient(circle_at_85%_18%,rgba(2,41,119,0.18),transparent_22%),linear-gradient(180deg,rgba(11,18,32,0.94),rgba(15,23,42,0.98))]" />
            <LoginHeroCarousel />
          </section>
